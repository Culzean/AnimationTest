...
class Animate {
    // Constructor
    //catapult, bitmap graphics
    public catapult: createjs.BitmapAnimation;
    private catapultTween: createjs.Tween;
    private catapultVel: number = 200; pps
    ...
    //rock, vector graphics
    private rock: createjs.Shape;
    private start: createjs.Point = new createjs.Point(0, 0);
    private distance: number;
    private frame: number = 0;
    private frameCount: number = 100;
    //buttons
    private buttonsAnim: createjs.BitmapAnimation;
    private buttons: createjs.ButtonHelper;

    private ease: Function;
    private crtEase: number = 0;

    constructor(model:GameModel, view:View) {
        p = this;
        this.model = model;
        this.view = view;
    }
    ...
    public Init():void {
        ...
        var img: HTMLImageElement = this.model.getAsset("catapult");
    ...
    public catapultCycle(name:string): void {
        ...
        switch(p.action){
            ...
            case "fire_off_":
                ...
                p.animation = p.action + p.direction;
                p.catapult.stop();
                //p.catapult.gotoAndStop(p.animation);
    ...
    public tick(dt: number, paused: boolean): void {
        ...
        if (p.rock.isVisible()) {
            //incr animation
            //Not sure if this is the best way to use the tween
            //choose veclocity
            //set incr for each step taken.
            

            //think there is betters ways to use tweenjs
            //but just getting the hang of things right now
            if (p.crtEase == 0) {
    ...
    public handleButtonClick(event): void {
    ...
    public handleMouseUp(event): void {

        if (p.buttonsAnim.currentAnimation == "out") {
            if (p.action == "ready_" || p.action == "move_") {
                //time, but in ms
                var time: number = 1000 * (Math.abs(p.view.stage.mouseX - p.catapult.x) / p.catapultVel);

                p.catapultTween = createjs.Tween.get(p.catapult, { loop: false }).
                    to({ x: p.view.stage.mouseX, y: p.catapult.y }, time, createjs.Ease.backOut);
                p.view.stage.update();
            }
        }
    }

    public handleReady(tween): void {
        p.action = "ready_";
    }
}
